...
const EmailVerificationForm = () => {
    ...
    const HandleEmailVerification = async () => {
        try {
            ...
            const response = await axios.post(`${BaseApiUrl}/api/user/reset-password`, { email: EmailRef.current?.value }, {
                withCredentials: true,
                headers: {
                }
            })

            if (response.data.message === "An email has been sent to you account with the reset link .") {
                toast.info("An email has been sent to you account with the reset link .")
                setMessage('An email has been sent to you account with the reset link .');
            }
            setIsPending("idle");
    ...
    return (<>
        <div className="h-screen flex items-center justify-center relative z-[2]  ">
            ...
            dark:from-black dark:to-white/10 grid grid-cols-1  py-6 px-4 rounded-lg gap-4 w-4/5 md:w-1/3 lg:w-1/3 shadow-sm shadow-black border dark:border-gray-400 cursor-grab">
                ...
                <span className="text-xs text-gray-700 dark:text-gray-300 space-grotesk text-center">verification is the first step</span>
                ...
                {message && <div className='w-full  p-2 text-white text-center text-md '>
                    {message}
                </div>}
                {/* buttons */}
                <span className="flex items-center justfify-center py-4 flex-col gap-2">
                    <motion.button disabled={isPending !== 'idle'} onClick={HandleEmailVerification} whileHover={{ boxShadow: "2px 2px 2px black", transform: "translateY(-3px)" }} whileTap={{ scale: 0.9, boxShadow: "2px 2px 2px blue" }} transition={{ duration: 0.2 }} className="bg-black dark:bg-white py-2 px-3 rounded-lg space-grotesk dark:text-black text-white w-full CustPoint flex items-center justify-center gap-2">{isPending === 'idle' ? (<>Get Code <IoMdCode /></>) : (<>Sending <IoIosHourglass className="animate-spin" /></>)}</motion.button>
