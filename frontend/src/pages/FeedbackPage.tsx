...
const Feedback = () => {
    ...
    return (<>
        <div className="h-screen flex items-center justify-center relative z-[2]">
            ...
            <div className="absolute h-full w-full top-0 left-0 bg-gradient-to-br from-blue-800/30 to-cyan-600/20 blur-2xl z-[-1]"></div>

            <motion.div
                drag
                whileDrag={{ scale: 0.9 }}
                dragConstraints={{ top: 0, left: 0, right: 0, bottom: 0 }}
                className="bg-gradient-to-br from-gray-100 to-gray-200 grid grid-cols-1 py-6 px-4 rounded-lg gap-4 w-4/5 md:w-1/3 lg:w-1/3 shadow-sm shadow-black curso-grab"
            >
                <h1 className="text-center space-grotesk font-bold text-2xl">Share Your Thoughts</h1>
                <span className="text-xs text-gray-700 space-grotesk text-center">We value your feedback!</span>
                ...
                <span className="flex items-center justify-center py-4 flex-col gap-2">
                    <motion.button disabled={isSending === 'pending'} onClick={HandleFormSubmit}
                        ...
                        className={`${isSending === "pending" ? "bg-gray-700" : "bg-black"} py-2 px-3 rounded-lg space-grotesk text-white w-full CustPoint flex items-center justify-center gap-2`}
                    >
                    ...
                    <motion.button
                        ...
                        transition={{ duration: 0.2 }}
                        className="bg-gray-400 py-2 px-3 rounded-lg space-grotesk text-black w-full flex items-center justify-center gap-2 CustPoint"
                    >
                        Rate Us <FiStar />
                    </motion.button>
                </span>
