...
const SubCategories = () => {
    ...
    return (<>
        <div className="  z-[1] space-grotesk w-auto md:w-fit  ">
            <label onClick={() => dispatch(setShowSubcategory(!showSubcategory))} className={`CustPoint  text-sm flex items-center justify-center gap-1 ${subCategory ? "text-pink-700" : "text-black dark:text-white"}`} htmlFor="Category">{subCategory ? subCategory : "Subcategory"} <IoMdArrowDropdown size={24} className={`transition-all duration-500  font-semibold ${showSubcategory === true ? "rotate-90" : "rotate-0"} `} /></label>
