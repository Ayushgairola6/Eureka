...
const DropDown: React.FC<DropDownProps> = ({ isVisible, setIsVisible, setCategory, category }) => {
    ...
    return (<>
        ...
        <div className="  z-[1] space-grotesk w-auto md:w-fit  ">
            <label onClick={() => setIsVisible(!isVisible)} className={`CustPoint  text-sm flex items-center justify-center gap-1 ${category ? "text-purple-700" : "text-black"}`} htmlFor="Category">{category ? category : "Category"} <IoMdArrowDropdown color="black" size={24} className={`transition-all duration-500  font-semibold ${isVisible === true ? "rotate-90" : "rotate-0"} `} /></label>
