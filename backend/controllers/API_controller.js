...
export const Generate_API_keys = async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from("API_KEYS").select('user_id, API_KEY').eq('user_id', user_id);
        if (error) {
            // console.error(error)
            return res.status(500).json({ message: "Something went wrong!" });
        }

        if (Array.isArray(data) && data.length > 0 && data[0]?.user_id === user_id && data[0]?.API_KEY) {
            return res.status(200).json({ message: "You already have an API key assigned", key: data[0].API_KEY });
        }
